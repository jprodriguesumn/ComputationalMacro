...
#Use linear system type from QuantEcon to simulate time series
lss = LSS(A,B,H)
X_simul, _ = simulate(lss, simul_length);
xss = [disteq;0.0;θeq]
X_simul = X_simul + repmat(xss,1,simul_length)

###Create time series of various aggregates
...
p4 = plot(disGrid,dis[:,:,1],title="Steady state distribution",xlims = (0.0,20))
p = plot(p1,p2,p3,p4, layout = (2,2), legend=false, size = (1000,400))
p = plot!(titlefont=("Helvetica",8))
p = plot!(legendfont=("Helvetica",6))
...
p = plot!(titlefont=("Helvetica",8))
p = plot!(legendfont=("Helvetica",6),xlims=(0.0,20.0))
savefig(p,"exodistribution.pdf")
